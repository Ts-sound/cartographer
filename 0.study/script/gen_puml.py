...
def handle_mapping():
    """
    Handle the mapping case.
    """
    mapping_path = os.path.join(workpath, "cartographer", "mapping")
    mapping_out_path = os.path.join(
        workpath, "0.study", "cartographer", "assets", "puml", "mapping"
    )
    os.makedirs(mapping_out_path, exist_ok=True)

    # gen mapping.puml
    input_files = glob.glob(os.path.join(mapping_path, "**/*.h"), recursive=True)
    output_file = mapping_out_path + "/mapping.puml"
    gen_puml(input_files, output_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mapping", help="", type=bool, default=False)
    ...
    if parser.parse_args().common:
        ...
        handle_io()
    elif parser.parse_args().mapping:
        handle_mapping()
